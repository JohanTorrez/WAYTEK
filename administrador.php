                          ...
                          <div class="form-row">
                            <div class="form-group col-6"><label for="procesador">Procesador</label><select name="procesador" class="form-control" id="">
                                ...
                                while ($procesadores = $cons_procesador->fetch_array(MYSQLI_ASSOC)) {
                                  echo '<option value="' . $procesadores['id_procesador'] . '">' . $procesadores['marca_procesador'], ' ', $procesadores['nombre_procesador'] . '</option>';
                                }
                            ...
                            <div class="form-group col-6">
                              ...
                              <select name="almacenamiento" class="form-control" id="">
                                ...
                                while ($almacenamientos = $cons_almacenamiento->fetch_array(MYSQLI_ASSOC)) {
                                  echo '<option value="' . $almacenamientos['id_almacenamiento'] . '">' . $almacenamientos['marca_almacenamiento'], ' ', $almacenamientos['nombre_almacenamiento'] . ' ' . $almacenamientos['capacidad_almacenamiento'] . ' ' . $almacenamientos['unidad_almacenamiento'], '</option>';
                                }
                          ...
                          <div class="form-row">
                            <div class="form-group col-6"><label for="ram">Memoria RAM</label><select name="ram" class="form-control" id="">
                                ...
                                while ($ram = $cons_ram->fetch_array(MYSQLI_ASSOC)) {
                                  echo '<option value="' . $ram['id_ram'] . '">' . $ram['marca_ram'], ' ', $ram['nombre_ram'] . ' ' . $ram['capacidad_ram'] . ' GB ' . $ram['tipo_ram'], '</option>';
                                }
            ...
            <div class="row table-responsive">
              <table id="tabla_productos" class="table table-sm shadow-none ml-1 mr-0 mt-4 d-table table-borderless">
                ...
                <tbody class="mb-5 bg-white rounded border-bottom text-center">
                  <?php
                  $cons = $mysqli->query("SELECT id_producto, tienda.nombre_tienda,tipo_producto, portatil.marca_portatil, portatil.nombre_portatil, accesorio.nombre_accesorio,pc_escritorio.nombre_pc_escritorio, presupuesto.nombre_presupuesto, foto_producto, fecha_publicacion_producto  FROM producto INNER JOIN tienda on producto.id_tienda=tienda.id_tienda INNER JOIN portatil on producto.id_portatil=portatil.id_portatil INNER JOIN accesorio on producto.id_accesorio=accesorio.id_accesorio INNER JOIN pc_escritorio on producto.id_pc_escritorio=pc_escritorio.id_pc_escritorio INNER JOIN presupuesto ON producto.id_presupuesto=presupuesto.id_presupuesto ORDER BY fecha_publicacion_producto DESC");
                ...
                <tbody class="mb-5 bg-white rounded border-bottom text-center">
                  ...
                  var fecha_calculada=(fecha_componente.fromNow());
                    ...
                    echo '<td class="text-center d-inline p-2">
                      ...
                      <button class="btn btn-elimi text-white border-0 btn-sm my-2 eliminar_componente_btn">
                      <i class="fas fa-trash mx-1"></i>Eliminar</button>
                      </td>
                      </tr>';
                  }
                  if (!$cons) {
